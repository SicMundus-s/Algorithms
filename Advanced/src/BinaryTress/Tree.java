...
/**
 * Двоичное дерево: Время выполнения поиска, вставки и удаления - O(log N)
 *
 * Драевидная структура данных, на примере чисел. Каждый узел(лист) имеет ссылку на левый(меньший) и правый(больший) узел(лист)
 * root - представляет "корень дерева". Найти меньший элемент мы можем пройдя по левой части "дерева", правый по правой.
 *
 * Удобно использовать когда все данные приходят не в отсортированном порядке, иначе получится несбалонсированное дерево,
 * когда все элементы находятся на левой части или правой части, то есть числа 10 20 30 40 50 будут находиться на одной "ветке"
 * что приведёт к вырождению эффективности до O(N) для всех операций. Кроме того, балансировку дерева могут нарушить
 * очень маленькие или очень большие значения ключей, фактически блокирующие
 * возможность вставки с той или иной стороны от узла
 */
public class Tree {
